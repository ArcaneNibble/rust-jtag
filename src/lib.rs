        ...
        actions.into_iter().map(|action| {
            match action {
                ...
                JTAGAction::ShiftBits{..} => {
                    self.execute_stjtag_action(action)
                }

                JTAGAction::ShiftIR{ir, capture, pause} => {
                    self.execute_stjtag_action(
                        &JTAGAction::GoViaStates(vec![JTAGState::ShiftIR]));

                    let ret = self.execute_stjtag_action(
                        &JTAGAction::ShiftBits {
                            bits_tdi: ir.clone(),
                            capture: *capture,
                            tms_exit: true,
                    ...
                    if *pause {
                        self.execute_stjtag_action(
                            &JTAGAction::GoViaStates(vec![JTAGState::PauseIR]));
                    } else {
                        self.execute_stjtag_action(
                ...
                JTAGAction::ShiftDR{dr, capture, pause} => {
                    self.execute_stjtag_action(
                        &JTAGAction::GoViaStates(vec![JTAGState::ShiftDR]));
                    ...
                    if *pause {
                        self.execute_stjtag_action(
                            &JTAGAction::GoViaStates(vec![JTAGState::PauseDR]));
                    } else {
                        self.execute_stjtag_action(
                            &JTAGAction::GoViaStates(vec![JTAGState::RunTestIdle]));
                    }
